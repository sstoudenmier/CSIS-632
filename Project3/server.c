...
/* leave_handler function that handles when a client wants to leave the server and
no longer receive information from the multicaster */
void *leave_handler(struct registration_packet *packet_reg) {
	int newsock, i;
	newsock = ntohs(packet_reg->sockid);
	/* loop through and remove the global table entry that has a matching sockid */
	for(i = 0; i < 20; i++) {
		if(records[i].sockid == newsock) {
	...
	printf("\n Received LEAVE Request For SockID %d \n", newsock);
	close(newsock);
	/* leaving the thread */
	pthread_exit(0);
}
...
void *join_handler(struct global_table *rec) {
	...
	packet_reg.sockid = htons(newsock);
...
int main(int argc, char* argv[]) {
	...
	while(1){
		...
		/* check to see if it is a leave request */
		else if(ntohs(packet_reg.type) == 321) {
			pthread_create(&threads[0], NULL, (void*)leave_handler, &packet_reg);
			pthread_join(threads[0], &exit_value);
		}
	}
}
